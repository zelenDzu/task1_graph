...
public:
    ...
    graph(const unsigned int nodes_count)
    {
    ...
    void set_start(const unsigned int id)
    {
        if (id < nodes.size())
            start = nodes[id];
    }

    void set_end(const unsigned int id)
    {
        if (id < nodes.size())
            end = nodes[id];
    }

    void add_edge_single(const unsigned int start_id,
                         const unsigned int end_id,
                         const int value,
                         const edgeType type = DIRECTIONAL)
    {
    ...
    void add_edge_flow(const unsigned int start_id,
                       const unsigned int end_id,
                       const int flow,
                       const int capacity,
                       const edgeType type = DIRECTIONAL)
    ...
    void show(const bool res = false) const
    {
        const string alf = "ABCDEFGHIJKLMNOPQ";
    ...
    stack* find_shortest_path(const unsigned int from,
    ...
    edge* get_edge(const unsigned int from, const unsigned int to) const
    {
        int k = 0;
        edge* e = nullptr;
